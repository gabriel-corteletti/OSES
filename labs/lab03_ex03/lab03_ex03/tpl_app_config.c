...
/* Task TaskINIT identifier */
#define TaskINIT_id 3
CONST(TaskType, AUTOMATIC) TaskINIT = TaskINIT_id;
...
/* Message msg_receive identifier */
#define msg_receive_id 0
CONST(MessageIdentifier, AUTOMATIC) msg_receive = msg_receive_id;

/*=============================================================================
 * Declaration of send messages IDs
 */

/* Message msg_send identifier */
#define msg_send_id 0
CONST(MessageIdentifier, AUTOMATIC) msg_send = msg_send_id;
...
/*-----------------------------------------------------------------------------
 * Task TaskINIT descriptor
 */
#define APP_Task_TaskINIT_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task TaskINIT function prototype
 */

FUNC(void, OS_APPL_CODE) TaskINIT_function(void);
#define APP_Task_TaskINIT_STOP_SEC_CODE
 ...
 *
 */
#define APP_Task_TaskINIT_START_SEC_STACK
#include "tpl_memmap.h"
tpl_stack_word TaskINIT_stack_zone[256/sizeof(tpl_stack_word)];
#define APP_Task_TaskINIT_STOP_SEC_STACK
#include "tpl_memmap.h"

#define OS_START_SEC_VAR_UNSPECIFIED
#include "tpl_memmap.h"

#define TaskINIT_STACK { TaskINIT_stack_zone, 256 }

/*
 * Task TaskINIT context
 */
avr_context TaskINIT_int_context;
#define TaskINIT_CONTEXT &TaskINIT_int_context
...
/*
 * Static descriptor of task TaskINIT
 */
CONST(tpl_proc_static, OS_CONST) TaskINIT_task_stat_desc = {
  /* context                  */  TaskINIT_CONTEXT,
  /* stack                    */  TaskINIT_STACK,
  /* entry point (function)   */  TaskINIT_function,
  /* internal ressource       */  NULL,
  /* task id                  */  TaskINIT_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task TaskINIT
 */
VAR(tpl_proc, OS_VAR) TaskINIT_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
...
/*-----------------------------------------------------------------------------
 * Action of message msg_receive
 */
/*-----------------------------------------------------------------------------
 * ALWAYS filter of message object msg_receive
 */
#define OS_START_SEC_CONST_UNSPECIFIED
#include "tpl_memmap.h"

tpl_noparam_filter_desc msg_receive_filter = {
...
/*-----------------------------------------------------------------------------
 * Static internal receiving queued message object msg_receive
 */
 
#define OS_START_SEC_VAR_UNSPECIFIED
#include "tpl_memmap.h"

VAR(bool, OS_VAR) msg_receive_buffer[1];
VAR(bool, OS_VAR) msg_receive_last;

VAR(tpl_queue_dyn, OS_VAR) msg_receive_dyn_queue = {
...
CONST(tpl_internal_receiving_queued_mo, OS_CONST) msg_receive_message = {
  { /* data receiving mo struct   */
    ...
    /*  filter pointer          */  (tpl_filter_desc *)&msg_receive_filter
  },
  { /*  queue structure   */
    /*  pointer to the dynamic descriptor   */  &msg_receive_dyn_queue,
    /*  max size of the queue               */  1*sizeof(bool),
    /*  element size of the queue           */  sizeof(bool),
    /*  pointer to the buffer               */  (tpl_com_data *)msg_receive_buffer,
    /*  pointer to the last written value   */  (tpl_com_data *)&msg_receive_last
  }
};
...
tpl_receive_message_table[RECEIVE_MESSAGE_COUNT] = {
  (tpl_base_receiving_mo*)&msg_receive_message
};
...
/*-----------------------------------------------------------------------------
 * Static internal sending static message object msg_send
 */
CONST(tpl_internal_sending_mo, OS_CONST) msg_send_message = {
  { /* base message object       */
    /* sending function          */ tpl_send_static_internal_message
  },
  /* pointer to the receiving mo */ (tpl_base_receiving_mo *)&msg_receive_message
};
...
tpl_send_message_table[SEND_MESSAGE_COUNT] = {
  (tpl_base_sending_mo*)&msg_send_message
};
